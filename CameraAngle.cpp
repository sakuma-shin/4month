...
void CameraAngle::Initialize(const WorldTransform& worldTransform)
{
    ...
    translation_ = { 0.0f, 30.0f, -50.0f };  // 初期位置
...
void CameraAngle::Update()
{
   ...
   // cameraTarget_ = player_->GetWorldPosition();
    ...
    // キー入力で移動量を加算
    if (input_->PushKey(DIK_LEFT)) {
        rotation_.y += kCameraSpeed; // 左回転（反時計回り）
    }
    if (input_->PushKey(DIK_RIGHT)) {
        rotation_.y -= kCameraSpeed; // 右回転（時計回り）
    }
    ...
    // rotation_.y を使ってカメラ位置を極座標で計算
    translation_.x = cameraTarget_.x + radius * std::sin(rotation_.y);
    translation_.z = cameraTarget_.z + radius * std::cos(rotation_.y);
    translation_.y = cameraTarget_.y + 30.0f; // 高さを固定
