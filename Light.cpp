...
void Light::Update() {
	...
	/*	if (growtype_ == NO) {
			...
			if (map_->CheckCollisionRay(initialPos_, endPos, collisionPos)) {
				worldTransform_.translation_ = collisionPos;
				worldTransform_.scale_ = {fabs(collisionPos.x - initialPos_.x), 0.5f, fabs(collisionPos.z - initialPos_.z)};
				growtype_ = NO;
				isMapHit = true;
			}
		}*/
	...
	if (worldTransform_.scale_.z >= 1.0f && growtype_ == Right || growtype_ == Left) {
		...
		if (growtype_ == NO ) {
			...
			Vector3 collisionPos;
			if (map_->CheckCollisionRay(initialPos_, endPos, collisionPos)) {
				if (map_->CheckCollision(collisionPos) != 8) {
